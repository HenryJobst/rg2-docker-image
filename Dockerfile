...
RUN ln -snf /usr/share/zoneinfo/$(curl https://ipapi.co/timezone) /etc/localtime && \
  apt-get update && apt-get install -y apache2 php libapache2-mod-php nodejs git wget npm  && \
...
RUN cd /var/www/html && mkdir -p kartat && mkdir -p kartat/cache && \
  ...
  git clone https://github.com/HenryJobst/rg2.git && cd rg2 && npm install grunt --save-dev && grunt
